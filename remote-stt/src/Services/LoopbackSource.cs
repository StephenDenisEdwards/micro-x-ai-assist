...
public sealed class LoopbackSource : IDisposable
{
    ...
    public LoopbackSource(ILogger<LoopbackSource> log)
    {
        _log = log;
    }

    /// <summary>
    /// Returns the current number of bytes buffered in the loopback provider (for diagnostics).
    /// </summary>
    ...
    public IWaveProvider Start(MMDevice device)
    {
        ...
        {
            // Keep behavior unchanged for hypothesis testing (don't force silence)
