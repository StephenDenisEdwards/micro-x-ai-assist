...
public sealed class SpeechPushClient : IAsyncDisposable
{
        ...
        IOptions<AudioOptions> audioOpts)
    {
        ...
        // Prefer key from configuration (user secrets), fallback to environment variable
        var key = string.IsNullOrWhiteSpace(_opts.Key)
            ? Environment.GetEnvironmentVariable("AZURE_SPEECH_KEY")
            : _opts.Key;
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException("Speech key not configured. Set 'Speech:Key' in user secrets or AZURE_SPEECH_KEY env var.");

        if (string.IsNullOrWhiteSpace(_opts.Region))
            throw new InvalidOperationException("Speech region not configured. Set 'Speech:Region' in configuration.");

        if (string.IsNullOrWhiteSpace(_opts.Language))
            throw new InvalidOperationException("Speech recognition language not configured. Set 'Speech:Language' in configuration (e.g., 'en-US').");

        // Azure Speech push-stream requires RAW PCM: 16-bit, mono
        if (a.TargetChannels != 1 || a.TargetBitsPerSample != 16)
            throw new InvalidOperationException("AudioOptions must produce 16-bit mono PCM for Azure Speech push-stream (TargetChannels=1, TargetBitsPerSample=16).");

        var speechConfig = SpeechConfig.FromSubscription(key, _opts.Region);
        speechConfig.SpeechRecognitionLanguage = _opts.Language;

        // Use the same audio format as the resampler pipeline
    ...
    public SpeechPushClient SetChannelTag(string tag)
    {
